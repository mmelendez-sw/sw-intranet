...
export class PowerbiService {
  ...
  public async generateEmbedToken(reportId: string): Promise<PowerbiEmbedToken> {
    try {
      ...
      // 🔓 EMBED FOR YOUR ORGANIZATION APPROACH: Use PowerBI's organization embed
      // This approach uses "Embed for your organization" which should work for
      // authenticated users within the same organization
      ...
      const embedToken: PowerbiEmbedToken = {
        embedUrl: embedUrl,
        token: '', // No token needed for organization embed
      ...
      console.log('🔓 Using organization embed approach - should work for authenticated users');
      return embedToken;
    } catch (error) {
