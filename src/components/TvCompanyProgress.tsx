...
/**
 * Live Company Progress for /tv — Power BI embed (Lambda ROPC) + Salesforce gauges.
 * Root `/` keeps the static office-TV imagery so the lobby display is unchanged.
 */
...
const SALESFORCE_CURRENT_INVESTMENTS_URL = (() => {
  ...
  if (typeof injected === 'string' && injected.trim()) {
    return `${injected.trim().replace(/\/$/, '')}/api/salesforce/current-investments`;
  }
  const host = window.location.hostname;
  if (host === 'localhost' || host === '127.0.0.1') {
    return 'http://localhost:3001/api/salesforce/current-investments';
  }
  // Amplify rewrite proxies /api/* to the Lambda Function URL.
        ...
        <section className="salesforce-panel">
          ...
          {loading ? (
            ...
            <div className="closed-rent-gauge">
              ...
              >
                <path className="closed-rent-gauge-track" d="M 30 150 A 120 120 0 0 1 270 150" pathLength={100} />
                <path
  ...
  useEffect(() => {
    ...
    const loadSalesforceRows = async () => {
      try {
        ...
        if (isActive) setSalesforceRows(data.records || []);
      } catch (error) {
          ...
          <div className="main-content home-page-progress-main">
            ...
            >
              {embedConfig ? (
                ...
                <div style={{ color: '#666' }}>Loading PowerBI report...</div>
              )}
            </div>
